...
- **On a DLC page the search goes to its base game.** PCGamingWiki documents DLC inside the game and has no page of its own for them. GG.deals names the game in the warning on the page itself ("This DLC requires base game …"), and the breadcrumb is the fallback: the entry right before the `DLCs` segment. The edition suffix (Standard, Deluxe, Premium…) comes off everywhere, for the same reason.
- **The link is a name search, not a direct page,** and the button says so in its tooltip. PCGamingWiki runs on MediaWiki and its search is imprecise, so it can land on a list of results or on the wrong game. Saying it up front stops that reading like a bug in the script.
...
- **En una ficha de DLC la búsqueda va a su juego base.** PCGamingWiki documenta los DLC dentro del juego y no tiene página propia para ellos. GG.deals nombra el juego en el aviso de la propia ficha ("This DLC requires base game …"), y el respaldo son las migas de pan: la entrada justo antes del segmento `DLCs`. El sufijo de edición (Standard, Deluxe, Premium…) se quita en todas, por el mismo motivo.
- **El enlace es una búsqueda por nombre, no una página directa,** y el botón lo dice en su tooltip. PCGamingWiki corre sobre MediaWiki y su buscador es impreciso, así que puede caer en una lista de resultados o en el juego equivocado. Decirlo de antemano evita que eso se lea como un fallo del script.
